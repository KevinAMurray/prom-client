...
exports.setValue = function setValue(hashMap, value, labels, timestamp) {
	...
	if (hashMap[hash]) {
		hashMap[hash].value = v;
		hashMap[hash].timestamp = t;
	} else {
		hashMap[hash] = {
			value: v,
			labels: labels || {},
...
function labelsAsPrometheusString(labels) {
	let labelsAsString = '';
	for (const key of Object.keys(labels)) {
		labelsAsString += `${key}="${escapeLabelValue(labels[key])}",`;
	}
	return labelsAsString;
}
exports.labelsAsPrometheusString = labelsAsPrometheusString;
